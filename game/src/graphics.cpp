...
void Graphics::blitSurface(SDL_Texture* texture, SDL_Rect* srcrect,
                           SDL_Rect* dstrect) {
...
void Graphics::create(int w, int h) {
    SDL_CreateWindowAndRenderer(w, h, SDL_RENDERER_ACCELERATED, &window,
                                &renderer);
}
...
SDL_Surface* Graphics::loadImage(const std::string& file) {
    ...
    if (images.count(file) == 0) {
        ...
        images[file] = SDL_CreateRGBSurfaceFrom(data, width, height, depth,
                                                channels * width, rmask, gmask,
                                                bmask, amask);
    }
